function onWayInvoiceQuery(e) {

  let tgt = getTarget(e);
  var postStr;
  postStr = 'operation=' + encodeURIComponent('select');
  if (tgt.id == 'onway_invoice_query_all') {
  ...
  xhr.onload = function () {
    if (xhr.status == 200) {
      ...
      $('#onway_invoice_results').show();
      $('#export').show();
    }
  };

  xhr.open("POST", "/handle_onway_invoices", true);
  xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
  xhr.send(postStr);
  console.log(postStr);
};
$('#onway_invoice_results').hide();
$('#export').hide();
$('#export').click(function(){
  table2xlsx('xlsx','onway_invoice_results');
});
...
var el = document.getElementById('onway_invoice_query_some');
...
var el2 = document.getElementById('onway_invoice_query_all');
